...
use crate::{
  game::{ClientState, ObjectPool, PlayerHandle, ServerState},
...
impl Game {
  ...
  pub fn server_apply<T>(
    ...
    mut perform: impl FnMut(&ClientState, &mut ObjectPool) -> ServerResult<T>,
    ...
    for<'de> T: Deserialize<'de>,
  {
    if self.server.decisions.contains(decision_handle) {
      ...
      if #[cfg(feature="server")] {
        let result = perform(&self.client, &mut self.server.objects);
  ...
  pub fn wrap_decision<T>(
    ...
    decision_handle: DecisionHandle,
    question: impl ToString,
    mut perform: impl FnMut(&ClientState, &mut ObjectPool) -> (T, Vec<T>),
    ...
    for<'de> T: Deserialize<'de>,
  {
    if self.server.decisions.contains(decision_handle) {
      ...
      cfg_if::cfg_if! {
        if #[cfg(feature="server")] {
          let (server_value, player_values) = perform(&self.client, &mut self.server.objects);
          let decision = self
            .server
            .decisions
  ...
  pub fn wrap_decision_public<T>(
    ...
    decision_handle: DecisionHandle,
    question: impl ToString,
    prepare: impl Fn(&ClientState, &mut ObjectPool) -> T,
    ...
    for<'de> T: Deserialize<'de>,
  {
    if self.server.decisions.contains(decision_handle) {
      ...
      cfg_if::cfg_if! {
        if #[cfg(feature="server")] {
          let value = prepare(&self.client, &mut self.server.objects);
